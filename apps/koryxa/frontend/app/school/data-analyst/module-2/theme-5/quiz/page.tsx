...
export default function Module2Theme5QuizPage() {
  ...
  useEffect(() => {
    async function load() {
      setMessage(null);
      try {
        const statusResp = await fetch(`${INNOVA_API_BASE}/school/data-analyst/module-2/theme-5/status`, { credentials: "include" });
        const statusData = await statusResp.json().catch(() => ({}));
        const hasSubmission = Boolean(statusData?.has_submission);
        setLocked(!hasSubmission);
        if (!hasSubmission) return;

        const quizResp = await fetch(`${INNOVA_API_BASE}/school/data-analyst/module-2/theme-5/quiz`, { credentials: "include" });
  ...
  async function submit() {
    ...
    setMessage(null);
    try {
      const payload = { answers: questions.map((q) => ({ question_id: q.id, answer_index: answers[q.id] })) };
      const resp = await fetch(`${INNOVA_API_BASE}/school/data-analyst/module-2/theme-5/quiz/submit`, {
        method: "POST",
        credentials: "include",
  ...
  if (locked) {
    return (
      <section className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
        <h1 className="text-2xl font-semibold text-slate-900">Quiz — Module 2 · Thème 5</h1>
        <p className="mt-2 text-sm text-slate-600">Quiz verrouillé : soumets d’abord un ZIP (même si rejeté).</p>
        {message && (
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
        <h1 className="text-2xl font-semibold text-slate-900">Quiz — Module 2 · Thème 5</h1>
        <p className="mt-2 text-sm text-slate-600">Les réponses viennent uniquement de quality_report.json + manifest.json.</p>
                ...
                {q.options.map((opt, optIdx) => (
                  ...
                  >
                    <input
                      ...
                      onChange={() => setAnswers((p) => ({ ...p, [q.id]: optIdx }))}
                      className="mt-1"
                    />
                    <span>{opt}</span>
                  </label>
