    ...
    sections: [
      {
        ...
        body: [
          ...
          "Un bon analyste ne se définit pas par sa capacité à manipuler un dataset, mais par sa capacité à clarifier trois choses : Quelle décision on cherche à améliorer ; Qui va utiliser le résultat (et qui peut bloquer le projet) ; Comment on va mesurer que l’analyse a vraiment servi.",
          "L’objectif n’est pas de produire des chiffres. L’objectif est de réduire l’incertitude qui bloque une action. Exemples : une équipe hésite entre deux offres ; un service observe une chute de performance ; une direction veut investir mais ne sait pas où. Dans tous ces cas, les données sont un moyen, pas une finalité.",
          "Quand on ne cadre pas, on tombe dans des pièges classiques : “On veut tout mesurer”, “On ne sait pas qui est le vrai client”, “On découvre les contraintes trop tard”, “Le projet n’est pas adopté”.",
          "Ce thème te force à répondre dès le départ : Pourquoi on fait cette analyse ? Pour qui ? Quelle décision ? Quels comportements on veut changer ? Quels KPI prouvent que ça marche ?",
          "Exemple KORYXA School : “On veut un dashboard sur les cours.” La vraie question est : quelle décision doit être améliorée ? (modules à améliorer, abandon au thème 2, canal d’acquisition, type d’exercice…). “Faire un dashboard” n’est pas une décision. C’est un format.",
          "À la fin, tu dois pouvoir produire : (1) un cadrage orienté décision (1 phrase vendable) ; (2) une première cartographie des parties prenantes (qui décide, qui utilise, qui peut bloquer…).",
          "À retenir : Si tu ne sais pas qui veut quoi et quelle décision est en jeu, tu fais de la technique. Si tu le sais, tu fais de la Data Analysis utile.",
          "Mini-checklist : Quelle décision veut-on améliorer ? Qui prend cette décision ? Qui va utiliser le résultat au quotidien ? Quel est l’impact attendu si on réussit ? Quel indicateur prouve le succès ? Quelles contraintes peuvent bloquer (accès data, délai, conformité, IT, terrain) ?",
        ],
      },
    ],
  },
