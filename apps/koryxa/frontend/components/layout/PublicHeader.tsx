...
export default function PublicHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-white/50 bg-white/72 backdrop-blur-2xl transition-colors dark:border-slate-800/80 dark:bg-slate-950/78">
      <div className="mx-auto flex w-full max-w-[var(--marketing-max-w)] items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
        <Link href="/" className="flex min-w-0 items-center gap-3">
          <div className="flex h-12 w-12 items-center justify-center rounded-[18px] border border-white/70 bg-[linear-gradient(145deg,#081226,#0b4b6f_58%,#0ea5e9)] shadow-[0_18px_40px_rgba(2,132,199,0.22)]">
            <span className="text-sm font-black tracking-[0.22em] text-white">K</span>
          </div>
          <div className="min-w-0">
            <p className="text-base font-black tracking-[0.08em] text-slate-950 dark:text-white">KORYXA</p>
            <p className="hidden text-[11px] text-slate-500 dark:text-slate-400 md:block">Plateforme premium • besoins IA, trajectoires, réseau et produits</p>
          </div>
        </Link>
            ...
            return (
              <Link
                ...
                className={clsx(
                  ...
                  active
                    ? "border-sky-200/90 bg-sky-50/90 text-sky-700 shadow-[0_10px_22px_rgba(14,165,233,0.08)] dark:border-sky-400/50 dark:bg-sky-500/15 dark:text-sky-100"
                    : "border-transparent text-slate-700 hover:border-slate-200/80 hover:bg-white/80 hover:text-slate-950 dark:text-slate-300 dark:hover:border-slate-700 dark:hover:bg-slate-900 dark:hover:text-white",
                )}
              >
                {link.label}
              </Link>
        ...
        <div className="hidden items-center gap-2 xl:flex">
          ...
          {!isAuthenticated ? (
            <Link
              href={loginHref}
              className="inline-flex whitespace-nowrap items-center justify-center rounded-full border border-slate-200/80 bg-white/90 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-sky-200 hover:text-sky-700 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:hover:border-sky-400/60 dark:hover:text-sky-100"
            >
              Se connecter
            </Link>
          ) : null}
          ...
          <Link
            href={platformHref}
            className={clsx(
              "inline-flex whitespace-nowrap items-center justify-center rounded-full px-5 py-2.5 text-sm font-semibold text-white transition",
              isAuthenticated
