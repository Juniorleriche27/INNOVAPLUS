...
export default function Module2Theme1Quiz() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const resp = await fetch(`${INNOVA_API_BASE}/school/data-analyst/module-2/theme-1/status`, { credentials: "include" });
  ...
  useEffect(() => {
    async function loadQuiz() {
      if (!validated) return;
      try {
        const resp = await fetch(`${INNOVA_API_BASE}/school/data-analyst/module-2/theme-1/quiz`, { credentials: "include" });
        const data = await resp.json().catch(() => ({}));
        if (!resp.ok) {
  ...
  async function submit() {
    ...
    try {
      ...
      const resp = await fetch(`${INNOVA_API_BASE}/school/data-analyst/module-2/theme-1/quiz/submit`, {
        method: "POST",
        credentials: "include",
  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
      ...
      <div className="mt-6 flex flex-wrap items-center gap-3">
        <button className="rounded-full bg-slate-900 px-5 py-2 text-sm font-semibold text-white" onClick={submit}>
          Valider le quiz
        </button>
        {result ? (
