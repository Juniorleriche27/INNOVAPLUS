...
export default function Module3Theme2Quiz() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const resp = await fetch(`${INNOVA_API_BASE}/school/data-analyst/module-3/theme-2/status`, { credentials: "include" });
  ...
  useEffect(() => {
    async function loadQuiz() {
      if (!validated) return;
      try {
        const resp = await fetch(`${INNOVA_API_BASE}/school/data-analyst/module-3/theme-2/quiz`, { credentials: "include" });
        const data = await resp.json().catch(() => ({}));
        if (!resp.ok) {
  ...
  async function submit() {
    ...
    try {
      ...
      const resp = await fetch(`${INNOVA_API_BASE}/school/data-analyst/module-3/theme-2/quiz/submit`, {
        method: "POST",
        credentials: "include",
  ...
  if (!validated) {
    return (
      <section className="rounded-3xl border border-amber-200 bg-amber-50 p-8 text-sm text-amber-900">
        Quiz verrouillé. Soumets d'abord les preuves du Thème 2.
        <div className="mt-4">
          <Link className="font-semibold text-sky-700" href="/school/data-analyst/module-3/theme-2/submit">
  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
      <h1 className="text-2xl font-semibold text-slate-900">Quiz Thème 2</h1>
      <p className="mt-2 text-sm text-slate-600">Questions basées sur ton `quality_report.json` + audits CSV.</p>
