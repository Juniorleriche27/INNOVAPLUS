...
export default function Theme1QuizPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const quizResp = await fetch(`${INNOVA_API_BASE}/school/data-analyst/module-1/theme-1/quiz`, {
          credentials: "include",
        });
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const resp = await fetch(`${INNOVA_API_BASE}/school/data-analyst/module-1/theme-1/quiz/submit`, {
        method: "POST",
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
        ...
        <p className="mt-2 text-sm text-slate-600">Questions basees sur ton notebook · seuil 70%</p>
      </section>

      {status === "locked" && (
        <section className="rounded-3xl border border-amber-200 bg-amber-50 p-6 text-sm text-amber-800">
          Quiz verrouille. Soumets d'abord les preuves du notebook dans la page Soumettre.
        </section>
      )}
