...
export default function Module2Theme3QuizPage() {
  ...
  useEffect(() => {
    async function load() {
      setMessage(null);
      try {
        const statusResp = await fetch(`${INNOVA_API_BASE}/school/data-analyst/module-2/theme-3/status`, {
          credentials: "include",
        });
        ...
        const quizResp = await fetch(`${INNOVA_API_BASE}/school/data-analyst/module-2/theme-3/quiz`, {
          credentials: "include",
        });
  ...
  async function submit() {
    ...
    try {
      ...
      const resp = await fetch(`${INNOVA_API_BASE}/school/data-analyst/module-2/theme-3/quiz/submit`, {
        method: "POST",
        credentials: "include",
  ...
  if (locked) {
    return (
      <section className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
        <h1 className="text-2xl font-semibold text-slate-900">Quiz — Module 2 · Thème 3</h1>
        <p className="mt-2 text-sm text-slate-600">Quiz verrouillé : soumets d’abord des livrables validés.</p>
        {message && (
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
        <h1 className="text-2xl font-semibold text-slate-900">Quiz — Module 2 · Thème 3</h1>
        <p className="mt-2 text-sm text-slate-600">Les réponses viennent du run report JSON + des exports CSV.</p>
