...
export default function PublicHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-white/55 bg-white/76 backdrop-blur-2xl transition-colors dark:border-slate-800/80 dark:bg-slate-950/78">
      <div className="mx-auto flex w-full max-w-[var(--marketing-max-w)] items-center justify-between gap-5 px-4 py-5 sm:px-6 lg:px-8">
        <Link href="/" className="flex min-w-0 items-center gap-4">
          <div className="flex h-14 w-7 items-center justify-center rounded-[15px] border border-white/75 bg-[linear-gradient(180deg,#05111d_0%,#08304f_55%,#0ea5e9_100%)] shadow-[0_18px_34px_rgba(2,132,199,0.2)]">
            <span className="text-base font-black tracking-[-0.08em] text-white">K</span>
          </div>
          ...
          {PUBLIC_NAV_LINKS.map((link) => {
            ...
            return (
              <Link
                ...
                href={link.href}
                className={clsx(
                  "whitespace-nowrap rounded-full border px-5 py-3 text-[1.02rem] font-semibold transition",
                  active
        ...
        <div className="hidden items-center gap-2.5 xl:flex">
          ...
          {!isAuthenticated ? (
            <Link
              href={loginHref}
              className="inline-flex whitespace-nowrap items-center justify-center rounded-full border border-slate-200/80 bg-white/92 px-5 py-3 text-[1.02rem] font-semibold text-slate-700 transition hover:border-sky-200 hover:text-sky-700 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:hover:border-sky-400/60 dark:hover:text-sky-100"
            >
              Se connecter
            </Link>
          ) : null}
          ...
          <Link
            href={platformHref}
            className={clsx(
              "inline-flex whitespace-nowrap items-center justify-center rounded-full px-7 py-3.5 text-[1.02rem] font-semibold text-white transition",
              isAuthenticated
