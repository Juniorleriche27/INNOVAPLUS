...
import { theme5Articles, theme5Meta, theme5Pages, theme5Videos } from "../../content";
    ...
    <div className="flex min-h-0 flex-col gap-6 overflow-hidden" key={pageNumber}>
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.25em] text-slate-500">{theme5Meta.module}</p>
            <h1 className="mt-2 text-2xl font-semibold text-slate-900">{theme5Meta.title}</h1>
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-600">
            ...
            <span className="rounded-full border border-slate-200 bg-white px-3 py-1">{theme5Meta.readingTime}</span>
          </div>
        </div>
        ...
        <div className="mt-5 flex flex-wrap items-center gap-2 text-sm">
          {pageNumber === 1 ? (
            ...
            <Link
              ...
              href={`/school/data-analyst/module-1/theme-5/page/${prevPage}`}
            >
              ← Page précédente
            </Link>
          )}
          ...
          {pageNumber === total ? (
            ...
            <Link
              ...
              href={`/school/data-analyst/module-1/theme-5/page/${nextPage}`}
            >
              Page suivante →
            </Link>
          )}
          ...
          <div className="ml-auto flex flex-wrap gap-2">
            <Link className="text-sky-700 hover:underline" href="/school/data-analyst/module-1/theme-5/notebook">
              Notebook
            </Link>
            <Link className="text-sky-700 hover:underline" href="/school/data-analyst/module-1/theme-5/submit">
              Soumettre
            </Link>
            <Link className="text-sky-700 hover:underline" href="/school/data-analyst/module-1/theme-5/quiz">
              Quiz
            </Link>
          </div>
        </div>
      ...
      <div className="grid h-[calc(100dvh-340px)] min-h-[420px] gap-6 overflow-hidden lg:grid-cols-[360px_minmax(0,1fr)] lg:gap-8">
        <aside className="space-y-6 overflow-y-auto overscroll-contain">
              ...
              {theme5Pages.map((entry, idx) => {
                ...
                return (
                  <Link
                    ...
                    href={`/school/data-analyst/module-1/theme-5/page/${idx + 1}`}
                  >
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-4">
              <VideoBlock videos={theme5Videos} />
            </div>
            <ul className="mt-5 space-y-2 text-sm text-slate-600">
              {theme5Articles.map((article) => (
                <li key={article.url}>
