...
export default function Module2Theme2QuizPage() {
  ...
  useEffect(() => {
    async function load() {
      setMessage(null);
      try {
        const statusResp = await fetch(`${INNOVA_API_BASE}/school/data-analyst/module-2/theme-2/status`, {
          credentials: "include",
        });
        ...
        const quizResp = await fetch(`${INNOVA_API_BASE}/school/data-analyst/module-2/theme-2/quiz`, {
          credentials: "include",
        });
  ...
  async function submit() {
    ...
    try {
      ...
      const resp = await fetch(`${INNOVA_API_BASE}/school/data-analyst/module-2/theme-2/quiz/submit`, {
        method: "POST",
        credentials: "include",
  ...
  if (locked) {
    return (
      <section className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
        <h1 className="text-2xl font-semibold text-slate-900">Quiz — Module 2 · Thème 2</h1>
        <p className="mt-2 text-sm text-slate-600">Quiz verrouillé : soumets d’abord les livrables validés.</p>
        {message && (
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
        <h1 className="text-2xl font-semibold text-slate-900">Quiz — Module 2 · Thème 2</h1>
        <p className="mt-2 text-sm text-slate-600">Certaines réponses viennent du JSON (preuve d’exécution).</p>
